...
ls_result_t ls_secure_storage_init(ls_secure_storage_t *secure,ls_storage_backend_t *storage,uint8_t *workspace,size_t workspace_size,size_t logical_capacity,const uint8_t key[LS_SECURITY_KEY_SIZE],uint32_t key_id,ls_crypto_random_fn random,void *random_context);
