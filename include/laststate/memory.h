...
typedef enum { LS_CAPTURE_METADATA, LS_CAPTURE_SNAPSHOT, LS_CAPTURE_STACK, LS_CAPTURE_SELECTIVE, LS_CAPTURE_FULL } ls_capture_level_t;
typedef enum { LS_REDACT_ZERO, LS_REDACT_HASH, LS_REDACT_EXCLUDE } ls_redaction_mode_t;
typedef struct { uint32_t free_bytes, minimum_free_bytes, largest_block, allocation_failures, pool_exhaustions; } ls_heap_stats_t;

ls_result_t ls_dump_region_register(const char *name, const void *address, size_t length, uint32_t flags);
