...
#include <stddef.h>
#include <stdint.h>
typedef struct {const char *name;void *handle;uintptr_t stack_pointer;uint32_t stack_high_watermark,runtime_counter,state;} ls_freertos_task_snapshot_t;
...
void ls_freertos_scheduler_state(uint32_t state,uint32_t task_count,uint32_t free_heap,uint32_t minimum_free_heap);
#endif
