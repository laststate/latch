...
#define CHECK(x)                                                                                   \
    do {                                                                                           \
        if (!(x)) {                                                                                \
            fprintf(stderr, "spool edges failed: %s:%d\n", #x, __LINE__);                          \
            return 1;                                                                              \
        }                                                                                          \
    } while (0)
...
static uint32_t test_record_crc(const test_spool_record_t *r) {
    return ls_crc32(r, offsetof(test_spool_record_t, header_crc));
}

static int setup(ls_storage_backend_t *storage, ls_memory_storage_t *memory) {
    (void)memory;
    static const ls_identity_t id = {
        .project_id = "spool-edge", .device_id = "host", .firmware_build_id = "spedge01"};
...
static int make_storage(ls_storage_backend_t *storage, ls_memory_storage_t *memory) {
    ...
    *storage = (ls_storage_backend_t){.name = "ram",
                                      .context = memory,
...
static int make_envelope(uint8_t *out, size_t *length) {
    ...
    return ls_envelope_encode(&e, out, LS_MAX_EVENT_SIZE, 1u, length) == LS_OK ? 0 : 1;
}

int main(void) {
    ls_storage_backend_t storage;
    ls_memory_storage_t memory;
    ...
    uint8_t env[LS_MAX_EVENT_SIZE];
    size_t env_len = 0u;
    CHECK(make_envelope(env, &env_len) == 0);
    ...
    CHECK(ls_spool_append(env, env_len, LS_PRIORITY_ERROR) ==
          LS_OK); /* no storage: capture remains non-blocking */

    memset(bytes, 0xff, sizeof bytes);
    make_storage(&storage, &memory);
    storage.capacity = 1u;
    ...
    storage.erase = NULL;
    CHECK(setup(&storage, &memory) == 0);
    CHECK(ls_spool_init() == LS_ECORRUPT);

    /* Erase geometry that cannot cover the complete spool is explicitly unsupported. */
    memset(bytes, 0, sizeof bytes);
    make_storage(&storage, &memory);
    storage.erase_size = 7u;
    CHECK(setup(&storage, &memory) == 0);
    CHECK(ls_spool_init() == LS_ENOTSUP);
    ...
    for (size_t i = 0; i < sizeof header_mutations / sizeof header_mutations[0]; ++i) {
        ...
        bytes[header_mutations[i]] ^= 1u;
        make_storage(&storage, &memory);
        CHECK(setup(&storage, &memory) == 0);
        CHECK(ls_spool_init() ==
              LS_OK); /* reformats invalid metadata when storage is recoverable */
    }
    ...
    CHECK(ls_spool_init() == LS_OK);
    CHECK(make_envelope(env, &env_len) == 0);
    CHECK(ls_spool_append(env, env_len, LS_PRIORITY_ERROR) == LS_OK);
    memcpy(baseline, bytes, sizeof bytes);
    ...
    for (size_t i = 0; i < sizeof rec_mutations / sizeof rec_mutations[0]; ++i) {
        memcpy(bytes, baseline, sizeof bytes);
        bytes[rec + rec_mutations[i]] ^= 0x80u;
        make_storage(&storage, &memory);
        CHECK(setup(&storage, &memory) == 0);
        CHECK(ls_spool_init() == LS_OK);
        CHECK(ls_spool_flush() == LS_OK);
    ...
    record->length = 0u;
    record->header_crc = test_record_crc(record);
    make_storage(&storage, &memory);
    CHECK(setup(&storage, &memory) == 0);
    CHECK(ls_spool_init() == LS_OK);
    CHECK(ls_spool_flush() == LS_OK);
    CHECK(ls_spool_get_stats(&stats) == LS_OK && stats.committed == 0u &&
          stats.corrupt_records == 0u);

    memcpy(bytes, baseline, sizeof bytes);
    record = (test_spool_record_t *)(void *)(bytes + rec);
    record->data_crc ^= 1u;
    record->header_crc = test_record_crc(record);
    make_storage(&storage, &memory);
    CHECK(setup(&storage, &memory) == 0);
    ...
    CHECK(setup(&storage, &memory) == 0);
    env[0] ^= 1u;
    CHECK(ls_spool_append(env, env_len, LS_PRIORITY_ERROR) == LS_ECORRUPT);
    return 0;
}
