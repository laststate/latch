...
static int test_util_and_metrics(void) {
    ...
    b[0] = 1;
    a[0] = 2;
    CHECK(ls_memcmp(a, b, 1) > 0);
    a[0] = 0;
    CHECK(ls_memcmp(a, b, 1) < 0);
    CHECK(ls_string_length(NULL) == 0 && ls_string_length("abc") == 3);
    CHECK(ls_hash_string(NULL) == 0);
    char text[4] = {'x', 'x', 'x', 'x'};
    ls_copy_string(text, sizeof text, "abcdef");
    CHECK(!strcmp(text, "abc"));
    ls_copy_string(text, sizeof text, NULL);
    CHECK(text[0] == '\0');
    ls_copy_string(NULL, 1, "x");
    ls_copy_string(text, 0, "x");
    ...
    CHECK(ls_writer_write(&writer, a, 0) == LS_ENOSPACE);
    writer.length = 0;
    CHECK(ls_writer_tlv(&writer, 1u, a, 7u) == LS_ENOSPACE);
    ...
    ls_metric_bool("flag", true);
    CHECK(ls_metric_get("flag", &snap) == LS_OK && snap.current == 1);
    ls_metric_u32("u", 12u);
    CHECK(ls_metric_get("u", &snap) == LS_OK && snap.current == 12);
    ls_metric_increment("ctr", 2u);
    ls_metric_increment("ctr", 3u);
    CHECK(ls_metric_get("ctr", &snap) == LS_OK && snap.current == 5 &&
          snap.type == LS_METRIC_COUNTER);
    ls_metrics_reset_window("ctr");
    CHECK(ls_metric_get("ctr", &snap) == LS_OK && snap.average == 0);
    ls_metrics_reset_window("missing");
    ls_metric_i32(NULL, 1);
    return 0;
}
...
static int test_breadcrumbs(void) {
    ...
    ls_breadcrumb_t event = {.category = "nav",
                             .level = LS_SEVERITY_ERROR,
                             .message_id = 7u,
                             .message = "fault",
                             .values = values,
    ...
    CHECK(ls_runtime
              .breadcrumbs[(ls_runtime.breadcrumb_next + LS_BREADCRUMB_CAPACITY - 1u) %
                           LS_BREADCRUMB_CAPACITY]
              .value_count == LS_BREADCRUMB_KV_MAX);
    ls_breadcrumb_event(NULL);
    ls_breadcrumb_set_min_level(LS_SEVERITY_DEBUG);
    for (size_t i = ls_runtime.breadcrumb_count; i < LS_BREADCRUMB_CAPACITY; ++i)
        ls_breadcrumb("fill");
    CHECK(ls_runtime.breadcrumb_count == LS_BREADCRUMB_CAPACITY);
    size_t next = ls_runtime.breadcrumb_next;
    ls_breadcrumb_set_policy(LS_BREADCRUMB_DROP_NEWEST);
    ls_breadcrumb("drop");
...
static int test_compression(void) {
    ...
    CHECK(ls_varint_u32_decode((uint8_t[]){0x80, 0x80, 0x80, 0x80, 0x10}, 5, &value, &consumed) ==
          LS_EOVERFLOW);
    CHECK(ls_varint_u32_decode((uint8_t[]){0x80, 0x80, 0x80, 0x80, 0x80}, 5, &value, &consumed) ==
          LS_EOVERFLOW);
...
static int test_secure_element(void) {
    ...
    e.random = se_ok_random;
    CHECK(ls_secure_element_random(&e, NULL, 1) == LS_EINVAL);
    ...
    e.sign_sha256 = se_ok_sign;
    CHECK(ls_secure_element_sign(&e, NULL, out) == LS_EINVAL);
    ...
    e.derive_key = se_ok_derive;
    CHECK(ls_secure_element_derive(&e, 1, NULL, 0, NULL) == LS_EINVAL);
...
static int test_network(void) {
    uint8_t env[LS_MAX_EVENT_SIZE], beacon[64];
    size_t env_len = 0, written = 0;
    ...
    ls_http_transport_t bad_http = http;
    bad_http.post = NULL;
    CHECK(ls_http_transport_send(&bad_http, env, env_len) == LS_EINVAL);
    bad_http = http;
    bad_http.endpoint = NULL;
    CHECK(ls_http_transport_send(&bad_http, env, env_len) == LS_EINVAL);
    CHECK(ls_http_transport_send(&http, NULL, env_len) == LS_EINVAL);
    CHECK(ls_http_transport_send(&http, env, 0) == LS_EINVAL);
    http.maximum_payload = env_len - 1u;
    CHECK(ls_http_transport_send(&http, env, env_len) == LS_ENOSPACE);
    http.maximum_payload = 0;
    CHECK(ls_http_transport_max_payload(&http) == LS_MAX_EVENT_SIZE);
    CHECK(ls_http_transport_max_payload(NULL) == LS_MAX_EVENT_SIZE);
    uint8_t bad[LS_MAX_EVENT_SIZE];
    memcpy(bad, env, env_len);
    bad[0] ^= 1u;
    CHECK(ls_http_transport_send(&http, bad, env_len) != LS_OK);
    mock.result = LS_EIO;
    CHECK(ls_http_transport_send(&http, env, env_len) == LS_EIO);
    mock.result = LS_OK;
    const uint16_t retry_codes[] = {408u, 425u, 429u, 500u, 503u};
    for (size_t i = 0; i < sizeof retry_codes / sizeof retry_codes[0]; ++i) {
        mock.status = retry_codes[i];
        CHECK(ls_http_transport_send(&http, env, env_len) == LS_EAGAIN);
    }
    mock.status = 404u;
    CHECK(ls_http_transport_send(&http, env, env_len) == LS_EIO);
    mock.status = 204u;
    CHECK(ls_http_transport_send(&http, env, env_len) == LS_OK);
    ...
    ls_mqtt_transport_t bad_mqtt = mqtt;
    bad_mqtt.publish = NULL;
    ...
    mqtt.qos = 3u;
    CHECK(ls_mqtt_transport_send(&mqtt, env, env_len) == LS_EINVAL);
    mqtt.qos = 1u;
    mqtt.maximum_payload = env_len - 1u;
    CHECK(ls_mqtt_transport_send(&mqtt, env, env_len) == LS_ENOSPACE);
    mqtt.maximum_payload = 0;
    CHECK(ls_mqtt_transport_max_payload(&mqtt) == LS_MAX_EVENT_SIZE);
    CHECK(ls_mqtt_transport_max_payload(NULL) == LS_MAX_EVENT_SIZE);
    CHECK(ls_mqtt_transport_send(&mqtt, bad, env_len) != LS_OK);
    mock.result = LS_EAGAIN;
    CHECK(ls_mqtt_transport_send(&mqtt, env, env_len) == LS_EAGAIN);
    mock.result = LS_OK;
    CHECK(ls_mqtt_transport_send(&mqtt, env, env_len) == LS_OK);

    CHECK(ls_incident_beacon_encode(NULL, env_len, 1, 12000, 20, beacon, sizeof beacon, &written) ==
          LS_EINVAL);
    CHECK(ls_incident_beacon_encode(env, env_len, 1, 12000, 20, beacon, 31, &written) == LS_EINVAL);
    CHECK(ls_incident_beacon_encode(bad, env_len, 1, 12000, 20, beacon, sizeof beacon, &written) !=
          LS_OK);
    CHECK(ls_incident_beacon_encode(env, env_len, 1, 12000, -3, beacon, sizeof beacon, &written) ==
          LS_OK);
    CHECK(written == 32u);
    return 0;
}
...
static int test_transport_defensive(void) {
    ...
    CHECK(ls_transport_result_is_retryable(LS_EAGAIN));
    CHECK(ls_transport_result_is_retryable(LS_EBUSY));
    CHECK(!ls_transport_result_is_retryable(LS_EIO));
    CHECK(!ls_transport_can_send(NULL, 1));
    ls_transport_backend_t none = {0};
    CHECK(!ls_transport_can_send(&none, 1));
    ls_transport_backend_t off = {.send = send_ok, .available = unavailable};
    CHECK(!ls_transport_can_send(&off, 1));
    ls_transport_backend_t z = {.send = send_ok, .max_payload = zero_mtu};
    CHECK(!ls_transport_can_send(&z, 1));
    ...
    ls_transport_backend_t reentrant = {.send = send_reentrant};
    reentrant_transport = &reentrant;
    CHECK(ls_transport_send(&reentrant, 1, data, sizeof data) == LS_OK);

    unsigned calls = 0;
    ls_transport_backend_t frag = {.send_fragment_v2 = fragment_retry,
                                   .context = &calls,
    ...
    ls_transport_fragment_t f = {.event_id = 1,
                                 .envelope_crc = ls_crc32(data, sizeof data),
                                 .total_length = 10,
                                 .fragment_capacity = 4,
                                 .fragment_crc = ls_crc32(data, 4),
                                 .fragment_index = 0,
                                 .fragment_count = 3,
                                 .data = data,
                                 .length = 4};
    ...
    f.total_length = 11;
    CHECK(ls_transport_reassembly_push(&r, &f, &length) == LS_ENOSPACE);
    f.total_length = 10;
    f.fragment_count = 17;
    CHECK(ls_transport_reassembly_push(&r, &f, &length) == LS_ENOSPACE);
    f.fragment_count = 2;
    CHECK(ls_transport_reassembly_push(&r, &f, &length) == LS_ECORRUPT);
    f.fragment_count = 3;
    f.offset = 1;
    CHECK(ls_transport_reassembly_push(&r, &f, &length) == LS_ECORRUPT);
    f.offset = 0;
    ...
    ls_transport_fragment_t other = f;
    other.event_id = 2;
    CHECK(ls_transport_reassembly_push(&r, &other, &length) == LS_EBUSY);
    CHECK(ls_transport_reassembly_push(&r, &f, &length) == LS_OK); /* identical duplicate */
    uint8_t changed[4] = {9, 9, 9, 9};
    other = f;
    other.data = changed;
    other.fragment_crc = ls_crc32(changed, 4);
    CHECK(ls_transport_reassembly_push(&r, &other, &length) == LS_ECORRUPT);
    CHECK(!r.active);
    ls_transport_reassembly_reset(NULL);
    return 0;
}
...
static int test_policy(void) {
    ...
    ls_policy_reset();
    for (size_t i = 0; i < LS_POLICY_CAPACITY; i++) {
        char name[16];
        snprintf(name, sizeof name, "p%zu", i);
        CHECK(ls_sampling_set_permyriad(name, 10000) == LS_OK);
    }
    CHECK(ls_sampling_set_permyriad("overflow", 10000) == LS_ENOSPACE);
    CHECK(ls_dedup_get(0, NULL) == LS_EINVAL);
    ls_dedup_snapshot_t ds;
    CHECK(ls_dedup_get(0, &ds) == LS_EINVAL);
    CHECK(!ls_policy_apply(NULL));
    ls_event_t critical = {.type = LS_EVENT_ERROR,
    ...
    ls_event_t e = {.type = LS_EVENT_ERROR,
                    .priority = LS_PRIORITY_WARNING,
                    .timestamp_ms = 10,
                    .domain = "rate",
                    .code = 1,
                    .message = "x"};
    CHECK(ls_policy_apply(&e));
    e.timestamp_ms = 11;
    e.code = 2;
    e.fingerprint = 0;
    CHECK(!ls_policy_apply(&e));
    ...
    e.fingerprint = 0;
    CHECK(ls_policy_apply(&e));
    e.timestamp_ms += LS_DEDUP_WINDOW_MS + 1u;
    e.fingerprint = critical.fingerprint;
    CHECK(ls_policy_apply(&e));
    ls_policy_reset();
    return 0;
}
...
int main(void) {
    static const ls_identity_t identity = {
        .project_id = "defensive", .device_id = "host", .firmware_build_id = "def00001"};
    ls_config_t config = {
        .identity = &identity, .enter_critical = enter_critical, .leave_critical = leave_critical};
