...
int main(void) {
    CHECK(ls_flash_mirror_physical_size_for_write(sizeof(workspace), ERASE_SIZE,
                                                  WRITE_SIZE) <= sizeof(physical));
    ...
    CHECK(mirror.backend.write(mirror.backend.context, 100u, old_value,
                               sizeof(old_value)) == LS_OK);
    memcpy(baseline, physical, sizeof(physical));

    flash.erase_calls = 0u;
    ls_storage_sim_fail_at(&simulator, 1u, 0u);
    CHECK(ls_flash_mirror_init(&mirror, &raw, workspace, sizeof(workspace)) ==
          LS_EIO);
    ...
    for (uint32_t operation = 1u; operation <= 140u; operation++) {
        ...
        (void)mirror.backend.write(mirror.backend.context, 100u, new_value,
                                   sizeof(new_value));
        ls_storage_sim_reset_faults(&simulator);
        ...
        CHECK(recovered.backend.read(recovered.backend.context, 100u, readback,
