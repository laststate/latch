...
#define CHECK(x)                                                                                   \
    do {                                                                                           \
        if (!(x)) {                                                                                \
            fprintf(stderr, "memory capture failed: %s:%d\n", #x, __LINE__);                       \
            return 1;                                                                              \
        }                                                                                          \
    } while (0)
...
static int encode_for_mode(ls_redaction_mode_t mode, bool redact, bool safe, visit_state_t *state,
    ...
    static const ls_identity_t identity = {
        .project_id = "mem", .device_id = "host", .firmware_build_id = "memcap01"};
    ...
    for (size_t i = 0; i < sizeof region; ++i)
        region[i] = (uint8_t)(i + 1u);
    if (safe) {
        CHECK(ls_dump_region_register("region", region, sizeof region, LS_DUMP_SAFE) == LS_OK);
    } else {
        /* Exercise the defensive skip of an unsafe descriptor that may originate in old retained
         * state. */
...
int main(void) {
    ...
    CHECK(ls_envelope_is_truncated(
        &info)); /* region is intentionally larger than configured maximum */
    ...
    for (size_t i = 16u; i < state.memory_length; ++i)
        CHECK(state.memory_value[i] == 0u);
    ...
    static const ls_identity_t identity = {
        .project_id = "stack", .device_id = "host", .firmware_build_id = "stack001"};
    ls_config_t config = {.identity = &identity};
    CHECK(ls_init(&config) == LS_OK);
    ...
    ls_arch_context_t cpu = {.architecture = LS_ARCH_CORTEX_M,
                             .msp = (uint32_t)(uintptr_t)(stack + 8u),
                             .psp = (uint32_t)(uintptr_t)(stack + 16u)};
    /* On 64-bit hosts ls_arch_context_t has 32-bit MCU SPs; directly set runtime bounds into the
       same low-width domain only if the host pointer can be represented. The positive path is
       otherwise physically exercised by Cortex-M HIL. */
