...
#define CHECK(x)                                                                                   \
    do {                                                                                           \
        if (!(x)) {                                                                                \
            fprintf(stderr, "fault pipeline failed: %s:%d\n", #x, __LINE__);                       \
            return 1;                                                                              \
        }                                                                                          \
    } while (0)
...
static bool available(void *c) {
    (void)c;
    return online != 0;
}
static size_t mtu(void *c) {
    (void)c;
    return LS_MAX_EVENT_SIZE;
}
static ls_result_t send_data(void *c, const uint8_t *d, size_t n) {
    (void)c;
    CHECK(d && n);
    sends++;
    return LS_OK;
...
static int setup(void) {
    static const ls_identity_t id = {.project_id = "fault-pipeline",
                                     .device_id = "auv",
                                     .firmware_build_id = "fault-pipeline-0001"};
...
static int capture_with_commit_failure(void) {
    ...
    ls_arch_context_t cpu = {
        .architecture = LS_ARCH_CORTEX_M, .fault = LS_FAULT_HARD, .pc = 1u, .lr = 2u};
...
static int storage_hook_is_real(void) {
    ...
    ls_storage_backend_t storage = {.context = &mem,
    ...
    CHECK(ls_storage_program(&storage, 0u, &value, 1u) == LS_EIO);
    CHECK(local[0] == 0xffu);
    ls_fault_injection_clear();
    CHECK(ls_storage_program(&storage, 0u, &value, 1u) == LS_OK);
    CHECK(local[0] == 0x7fu);
    return 0;
}

int main(void) {
    CHECK(capture_with_commit_failure() == 0);
    CHECK(send_and_ack_failures_are_retryable() == 0);
    CHECK(storage_hook_is_real() == 0);
    return 0;
}
