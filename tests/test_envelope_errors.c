...
int main(void) {
    ...
    CHECK(ls_boot() == LS_OK);
    ls_event_t encode_event = {.type = LS_EVENT_MESSAGE,
    ...
    CHECK(ls_envelope_validate(changed, captured_length, 0) == LS_ECORRUPT);
    memcpy(changed, captured, captured_length);
    changed[7] = LS_ENVELOPE_ENCRYPTED;
    CHECK(ls_envelope_validate(changed, captured_length, 0) == LS_ECORRUPT);
    memcpy(changed, captured, captured_length);
    changed[7] = LS_ENVELOPE_AEAD;
    CHECK(ls_envelope_validate(changed, captured_length, 0) == LS_ECORRUPT);
    memcpy(changed, captured, captured_length);
    changed[7] = LS_ENVELOPE_AUTHENTICATED;
    CHECK(ls_envelope_validate(changed, captured_length, 0) == LS_ECORRUPT);
    ...
    CHECK(ls_envelope_validate(changed, captured_length, 0) == LS_ECORRUPT);
    memcpy(changed, captured, captured_length);
    changed[LS_LEP_HEADER_SIZE + 4u] ^= 1u;
    CHECK(ls_envelope_validate(changed, captured_length, 0) == LS_ECORRUPT);
    ...
    malformed[LS_LEP_HEADER_SIZE + 0] = 1u;
    malformed[LS_LEP_HEADER_SIZE + 1] = 0u;
    malformed[LS_LEP_HEADER_SIZE + 2] = 5u;
    malformed[LS_LEP_HEADER_SIZE + 3] = 0u;
    write_u32(malformed + 20u, ls_crc32(malformed, 20u));
    ...
    CHECK(ls_envelope_decrypt_payload(invalid_public, captured_length, public_workspace,
                                      sizeof public_workspace, &invalid_length) == LS_ECORRUPT);
    CHECK(ls_envelope_visit(invalid_public, captured_length, accept_visit, NULL) == LS_ECORRUPT);
    CHECK(ls_envelope_visit_secure(invalid_public, captured_length, public_workspace,
                                   sizeof public_workspace, accept_visit, NULL) == LS_ECORRUPT);
    CHECK(ls_envelope_visit_secure_replay(invalid_public, captured_length, &invalid_replay,
                                          public_workspace, sizeof public_workspace, accept_visit,
                                          NULL) == LS_ECORRUPT);
    size_t plain_length = 0u;
    CHECK(ls_envelope_decrypt_payload(captured, captured_length, NULL, 0u, &plain_length) ==
          LS_EAUTH);
    ...
    unsigned visits = 0u;
    CHECK(ls_envelope_visit_secure(captured, captured_length, NULL, 0u, accept_visit, &visits) ==
          LS_EAUTH);
    ls_envelope_replay_t plaintext_replay = {0};
    CHECK(ls_envelope_visit_secure_replay(captured, captured_length, &plaintext_replay, NULL, 0u,
                                          accept_visit, &visits) == LS_EAUTH);
    ls_security_policy_t permissive = {.algorithm = LS_SECURITY_XCHACHA20_POLY1305,
                                       .key_id = 1u,
    ...
    CHECK(ls_envelope_visit_secure(captured, captured_length, NULL, 0u, accept_visit, &visits) ==
          LS_OK);
    CHECK(visits > 0u);
    ls_security_policy_t hmac = {.algorithm = LS_SECURITY_HMAC_SHA256,
                                 .key_id = 1u,
    ...
    CHECK(ls_envelope_visit_secure_replay(captured, captured_length, &hmac_replay, NULL, 0u,
                                          accept_visit, &visits) == LS_OK);
    CHECK(ls_envelope_visit_secure_replay(captured, captured_length, &hmac_replay, NULL, 0u,
                                          accept_visit, &visits) == LS_EAUTH);
    memcpy(changed, captured, captured_length);
    changed[captured_length - 1u] ^= 1u;
    CHECK(ls_envelope_visit_secure(changed, captured_length, NULL, 0u, accept_visit, &visits) ==
          LS_EAUTH);
