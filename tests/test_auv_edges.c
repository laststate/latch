...
#define CHECK(x) do { if (!(x)) { fprintf(stderr, "auv edge check failed: %s:%d\n", #x, __LINE__); return 1; } } while (0)

static uint32_t now_ms;
static uint8_t storage_bytes[64000];

static uint32_t clock_ms(void *context) { (void)context; return now_ms; }
static bool online(void *context) { (void)context; return true; }
static size_t max_payload(void *context) { (void)context; return LS_MAX_EVENT_SIZE; }
...
static ls_result_t random_ok(void *context, uint8_t *output, size_t length) {
    ...
    if (!output && length) return LS_EINVAL;
    for (size_t i = 0; i < length; ++i) output[i] = (*value)++;
    return LS_OK;
}
static ls_result_t random_fail(void *context, uint8_t *output, size_t length) {
    (void)context; (void)output; (void)length; return LS_EIO;
}

static ls_result_t self_ok(void *ctx) { (void)ctx; return LS_OK; }
static ls_result_t self_fail(void *ctx) { (void)ctx; return LS_EIO; }

typedef struct { unsigned destroy_calls; bool fail_destroy; } se_state_t;
static ls_result_t se_sign(void *ctx, const uint8_t digest[32], uint8_t signature[64]) {
    (void)ctx;
    if (!digest || !signature) return LS_EINVAL;
    for (size_t i = 0; i < 64; ++i) signature[i] = (uint8_t)(digest[i % 32] ^ 0x5au);
    return LS_OK;
}
static ls_result_t se_destroy(void *ctx, uint32_t key_id) {
    se_state_t *state = (se_state_t *)ctx;
    state->destroy_calls++;
    if (!key_id) return LS_EINVAL;
...
static int setup_with_identity(const ls_identity_t *identity) {
    ...
    transport = (ls_transport_backend_t){
        .name = "sink", .priority = 1u, .available = online, .send = sink,
...
static int setup(void) {
    static const ls_identity_t identity = {
        .project_id = "auv-edges", .device_id = "auv-edge-node", .product = "commercial-auv",
        .hardware_revision = "revZ", .firmware_version = "0.3.0",
        .firmware_build_id = "auv-edge-build-0001", .bootloader_version = "boot-test",
        .git_commit = "abcdef01", .architecture = "host", .rtos = "test",
    };
...
static int test_blackbox_edges(void) {
    ...
    for (size_t i = 0; i < LS_BLACKBOX_CAPACITY + 5u; ++i) {
        CHECK(ls_blackbox_record_values(LS_BLACKBOX_USER, (uint16_t)i, 0u, (int32_t)i, 0, 0, 0) == LS_OK);
    }
    CHECK(ls_blackbox_get_stats(&stats) == LS_OK);
    ...
    CHECK(ls_blackbox_record_values(LS_BLACKBOX_USER, 9u, LS_BLACKBOX_IMPORTANT, 0, 0, 0, 0) == LS_EBUSY);
    ls_blackbox_thaw();
    CHECK(ls_blackbox_record_values(LS_BLACKBOX_USER, 9u, LS_BLACKBOX_IMPORTANT, 0, 0, 0, 0) == LS_OK);
...
static int test_environment_health_edges(void) {
    ...
    ls_environment_sample_t a = {.timestamp_ms = 777u, .pressure_pa = 200000u, .depth_cm = 1000,
        .internal_temperature_c = 30, .humidity_permyriad = 5000u, .vibration_mg_rms = 500u};
    ls_environment_sample(&a);
    ls_environment_sample_t b = {.pressure_pa = 100000u, .depth_cm = 100,
        .internal_temperature_c = 20, .humidity_permyriad = 1000u, .vibration_mg_rms = 100u};
    ...
    ls_power_sample_t p1 = {.timestamp_ms = now_ms, .vdd_mv = 3300u, .battery_mv = 12000u,
                            .current_ma = -50, .temperature_c = 20};
    ls_power_sample_t p2 = {.timestamp_ms = now_ms + 1u, .vdd_mv = 2800u, .battery_mv = 11000u,
                            .current_ma = 500, .temperature_c = 40};
    ls_power_sample(&p1); now_ms += 1u; ls_power_sample(&p2);
...
static int test_supervisor_edges(void) {
    ...
    ls_supervisor_config_t cfg = {
        .watchdog_max_stale_ms = 10u, .minimum_vdd_mv = 3000u,
        .minimum_battery_mv = 10000u, .maximum_temperature_c = 60,
        .minimum_heap_free_bytes = 100u, .maximum_spool_percent = 1u,
        .maximum_vibration_mg_rms = 1000u, .anomaly_hold_ms = 20u,
    };
...
static int test_selftest_fault_injection_edges(void) {
    ...
    invalid.id = 1u; invalid.name = "bad";
    CHECK(ls_selftest_register(&invalid) == LS_EINVAL);
    ...
    first.name = "replaced"; first.run = self_fail;
    CHECK(ls_selftest_register(&first) == LS_OK);
    ...
    ls_selftest_case_t overflow = {.id = (uint16_t)(LS_SELFTEST_CAPACITY + 1u), .name = "overflow", .run = self_ok};
    ...
    for (size_t i = 1u; i < LS_FAULT_INJECTION_CAPACITY; ++i) ls_fault_injection_disarm(names[i]);
...
static int test_provisioning_edges(void) {
    ...
    ls_secure_element_t element = {.context = &se, .sign_sha256 = se_sign, .destroy_key = se_destroy};
    ...
    CHECK(ls_provisioning_attest(&element, challenge, sizeof(challenge), digest, signature) == LS_EINVAL);
...
static int test_fingerprint_trace(void) {
    ...
    ls_arch_context_t context = {.architecture = LS_ARCH_CORTEX_M, .fault = LS_FAULT_HARD,
        .pc = 0x12345678u, .lr = 0xabcdef01u, .cfsr = 3u, .hfsr = 4u};
