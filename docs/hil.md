...
The board firmware must accept `HIL:RUN:<scenario>` over its control UART, emit `HIL:ARMED:<SCENARIO>` immediately before the fault, reboot, inspect the retained Latch event and hardware reset registers, then emit `HIL:PASS:<SCENARIO>`. Supported scenarios are brownout, HardFault, watchdog, stack-canary corruption, MPU, TrustZone, lazy FPU stacking, real Flash and reset registers.

`hil/cortex_m_hil.c` supplies HardFault and stack-canary triggers plus a destructive Flash erase/program/readback check. The product fixture must place and verify its canary at the actual configured stack boundary; flipping an arbitrary word is not stack-overflow qualification. Brownout is performed externally by a SCPI power supply. The runner configuration follows `hil/boards/stm32u5.json.example`; keep real COM ports and lab addresses outside the repository.
