...
Mutation('crypto-kat-hkdf-bypass','src/security/provider.c','if (r != LS_OK || !bytes_equal(hkdf, hkdf_expected, sizeof hkdf)) return LS_EAUTH;','if (r != LS_OK && !bytes_equal(hkdf, hkdf_expected, sizeof hkdf)) return LS_EAUTH;','latch-crypto-commercial-tests','latch-crypto-commercial-tests'),
...
Mutation('crypto-kat-tag-bypass','src/security/provider.c','!bytes_equal(tag, tag_expected, sizeof tag)) return LS_EAUTH;','bytes_equal(tag, tag_expected, sizeof tag)) return LS_EAUTH;','latch-crypto-commercial-tests','latch-crypto-commercial-tests'),
...
Mutation('provision-rotation-same-key-allowed','src/core/provisioning.c','new_key_id == ls_runtime.persistent.provision_key_id || !monotonic_allowed(monotonic_counter)','new_key_id != ls_runtime.persistent.provision_key_id || !monotonic_allowed(monotonic_counter)','latch-auv-edges-tests','latch-auv-edges-tests'),
